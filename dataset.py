...
def prepare_dataset(num_partitions: int, batch_size: int, val_ratio: float = 0.1):
    ...
    # split train_set into `num_partitions` train_sets (one per client)
    ...
    # split randomly. This returns a list of train_sets, each with `num_images` training examples
    ...
    train_sets = random_split(
        train_set, partition_len, torch.Generator().manual_seed(2023)
    )
    ...
    for train_set_ in train_sets:
        num_total = len(train_set_)
        num_val = int(val_ratio * num_total)
        num_train = num_total - num_val

        for_train, for_val = random_split(
            train_set_, [num_train, num_val], torch.Generator().manual_seed(2023)
        )
    ...
    # this purpose and only use the test_set after the final round.
